...
class PDKVector:PDWeapon{
	...
	fire:
		TNT1 A 1{
			...
			A_PDBulletAttack(0.5,0.5,1,10 * random(1,2),"PDPistolPuff",flags:CBAF_NORANDOM);
			A_MuzzleClimb(0.11,0.22,false); // unbelievably low recoil tbh
			A_TakeInventory("PDSMGLoaded",1);
			A_PlaySound("weapons/pistol",CHAN_WEAPON);
		}
		TNT1 A 1 A_JumpIf(countinv("PDSMGLoaded") < 1,"ready");
	...
	reloading:
		TNT1 A 1{
			if(invoker.pdp.RoomscaleDistance(4.0,-3.0) < 3.0){
